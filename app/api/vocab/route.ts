...
export async function PATCH(req: NextRequest) {
  if (!isVocabDbEnabled()) {
    return NextResponse.json(
      { error: 'DATABASE_URL이 설정되어 있지 않아 단어장을 수정할 수 없습니다' },
      { status: 400 }
    )
  }
  ...
  if (typeof body?.completed !== 'boolean') {
    return NextResponse.json(
      { error: 'completed(boolean)가 필요합니다' },
      { status: 400 }
    )
  }

  try {
    await updateVocabCompleted(id, body.completed)
    return NextResponse.json({ ok: true })
  } catch (err) {
    console.error(err)
    return NextResponse.json({ error: '단어장 수정에 실패했습니다' }, { status: 500 })
  }
}
